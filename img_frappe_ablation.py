...
frappe_author = [98.47, 98.56, 98.51, 98.52, 98.47, 98.61]
frappe_self = [98.45, 98.47, 98.47, 98.41, 98.54, 98.69]
...
for i, method in enumerate(methods):
    # 为 'Author' 绘制柱状图
    ax.bar(index[i] - bar_width / 2, frappe_author[i], bar_width, color=author_colors[i],
           label=f'Author: {method}' if i == 0 else "")  # 仅第一次绘制时添加标签

    # 为 'Self' 绘制柱状图
    ax.bar(index[i] + bar_width / 2, frappe_self[i], bar_width, color=self_colors[i],
           label=f'Self: {method}' if i == 0 else "")  # 仅第一次绘制时添加标签

# 设置标题和标签
ax.set_title('Frappe AUC Comparison', fontsize=16, weight='bold')  # 加粗标题
...
# 设置纵坐标细粒度范围
ax.set_ylim(98.2, 98.8)
ax.yaxis.set_major_locator(MaxNLocator(integer=True, prune='lower', nbins=6))

# 标注 AUC 值
for i in range(len(methods)):
    ax.text(i - bar_width / 2, frappe_author[i] + 0.005, f'{frappe_author[i]:.2f}', ha='center', fontsize=10)
    ax.text(i + bar_width / 2, frappe_self[i] + 0.005, f'{frappe_self[i]:.2f}', ha='center', fontsize=10)
...
# 保存图像
plt.savefig('D:\\PapersCode\\FinalMLP\\result_csv\\frappe_auc_comparison', dpi=300)
