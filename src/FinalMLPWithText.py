...
class FinalMLPWithText(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="FinalMLP",
                 gpu=-1,
                 ...
                 **kwargs):
        super(FinalMLPWithText, self).__init__(feature_map,
                                       model_id=model_id,
                                       gpu=gpu,
    ...
    def forward(self, X):
        ...
        features = torch.cat([v for k, v in X.items() if k != 'text'], dim=1)
        # 其他特征通过嵌入层
        fea_emb = self.other_features_embedding(features)
        text_emb = text_features.mean(dim=1)  # 沿着第二个维度（时间步）求平均，结果维度为 (batch_size, 768)
        # 将文本特征和嵌入的其他特征拼接融合
        com_emb = torch.cat((fea_emb, text_emb ), dim=1)
        com_emb = self.fc(com_emb)
        if self.use_fs:
            feat1, feat2 = self.fs_module(X, com_emb)
        else:
            feat1, feat2 = com_emb, com_emb
