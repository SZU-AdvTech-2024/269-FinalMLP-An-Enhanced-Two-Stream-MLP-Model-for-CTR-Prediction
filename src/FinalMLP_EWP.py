...
class FinalMLP_EWP(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="FinalMLP_EWP",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=10,
                 ...
                 **kwargs):
        super(FinalMLP_EWP, self).__init__(feature_map,
                                       model_id=model_id,
                                       gpu=gpu,
        ...
        if self.use_fs:
            self.fs_module = FeatureSelection(feature_map,
                                              ...
                                              fs1_context,
                                              fs2_context)
        self.fusion_module = EWPFusion()  # 使用EWP融合
...
class EWPFusion(nn.Module):
    def __init__(self):
        super(EWPFusion, self).__init__()
        self.output_layer = None
        self.fc = nn.Linear(400, 1)  # 添加一个中间全连接层

    def forward(self, x, y):
        if self.output_layer is None:
            x_dim = x.shape[1]  # 获取x的特征维度
            y_dim = y.shape[1]  # 获取y的特征维度

            # 动态计算输入维度（x_dim * y_dim）
            input_dim = x_dim * y_dim
            self.output_layer = nn.Linear(input_dim, 1).to(x.device)
