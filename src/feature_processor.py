...
class FeatureProcessor(object):
    def __init__(self, feature_cols=[], label_col=[], dataset_id=None, data_root="../data/",
                 model_path="D:/PapersCode/FinalMLP2/bert-base-uncased", **kwargs):
        ...
        self.label_cols = label_col if isinstance(label_col, list) else [label_col]
        ...
        self.dtype_dict = {feat["name"]: eval(feat["dtype"]) if isinstance(feat["dtype"], str) else feat["dtype"]
                           for feat in self.feature_cols + self.label_cols}
        self.processor_dict = dict()
        
        # Load pre-trained model and tokenizer from local path
    ...
    def _complete_feature_cols(self, feature_cols):
        ...
        # 确保 text 列被添加到 feature_cols 中
        if not any(col["name"] == "text" for col in full_feature_cols):
            text_col = {
                "active": True,
                "dtype": "object",
                "name": "text",
                "type": "sequence"  # 假设 text 列是序列类型
            }
            full_feature_cols.append(text_col)
    ...
    def fit(self, train_ddf, min_categr_count=1, num_buckets=10, **kwargs):
        ...
        for col in self.feature_cols:
            name = col["name"]
            if col["active"]:
                logging.info("Processing column: {}".format(name))
                if col["type"] == "meta":  # e.g. group_id
                    ...
                    raise NotImplementedError("feature_col={}".format(name))
    ...
    def fit_sequence_col(self, col, col_values, min_categr_count=1, batch_size=32):
        name = col["name"]
        feature_type = col["type"]
        feature_source = col.get("source", "")
        self.feature_map.features[name] = {"source": feature_source, "type": feature_type}

        # 确保 col_values 是字符串列表
        if not isinstance(col_values, list):
            col_values = col_values.tolist()  # 将 Pandas Series 或 NumPy 数组转换为列表
        if not all(isinstance(item, str) for item in col_values):
            col_values = [str(item) for item in col_values]  # 将列表中的每个元素转换为字符串

        # 确定批次大小
        batch_size = min(batch_size, len(col_values))  # 确保批次大小不超过数据长度

        # 生成嵌入向量
        embeddings = []
        for i in range(0, len(col_values), batch_size):
            batch = col_values[i:i + batch_size]
        ...
        # 将嵌入向量存储到 self.processor_dict 中
        self.processor_dict[name + "::embeddings"] = np.concatenate(embeddings, axis=0)

        # 更新特征映射信息
        self.feature_map.features[name].update({
            "embedding_dim": self.model.config.hidden_size,
            "vocab_size": len(self.tokenizer),
            "oov_idx": self.tokenizer.vocab["[UNK]"],
            "padding_idx": self.tokenizer.vocab["[PAD]"],
            "max_len": 512  
        })
        self.processor_dict[name + "::tokenizer"] = self.tokenizer
    ...
    def transform(self, ddf):
        ...
        for feature, feature_spec in self.feature_map.features.items():
            if feature in ddf.columns:
                ...
                col_values = ddf[feature].astype(str).values  # 确保所有值都是字符串类型
                ...
                elif feature_type == "sequence" or feature_type == "text":  # 处理文本序列
                    inputs = self.tokenizer(col_values, return_tensors='pt', padding=True, truncation=True,
                                            max_length=512)
                    inputs = {k: v.to(self.device) for k, v in inputs.items()}
                    with torch.no_grad():
                        outputs = self.model(**inputs)
