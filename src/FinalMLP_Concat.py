...
class FinalMLP_Concat(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="FinalMLP_Concat",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=10,
                 ...
                 **kwargs):
        super(FinalMLP_Concat, self).__init__(feature_map,
                                       model_id=model_id,
                                       gpu=gpu,
        ...
        if self.use_fs:
            self.fs_module = FeatureSelection(feature_map,
                                              ...
                                              fs1_context,
                                              fs2_context)
        self.fusion_module = ConcatFusion()  # 使用连接融合
...
class ConcatFusion(nn.Module):
    ...
    def forward(self, x, y):
        # 在forward方法中动态确定input_dim
        if not self.initialized:
            # 假设x和y的特征维度分别是D1和D2
            D1, D2 = x.shape[1], y.shape[1]
            # 设置output_layer的输入特征维度为x和y的特征维度之和
            self.output_layer = nn.Linear(D1 + D2, 1)
        ...
        # 确保张量在正确的设备上
        if next(self.parameters()).device.type != z.device.type:
            self.output_layer = self.output_layer.to(z.device)

        # 通过线性层将形状从(batch_size, D1 + D2)调整为(batch_size, 1)
