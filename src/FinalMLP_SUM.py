...
class FinalMLP_SUM(BaseModel):
    def __init__(self,
                 feature_map,
                 model_id="FinalMLP_SUM",
                 gpu=-1,
                 learning_rate=1e-3,
                 embedding_dim=10,
                 ...
                 **kwargs):
        super(FinalMLP_SUM, self).__init__(feature_map,
                                           model_id=model_id,
                                           gpu=gpu,
        ...
        # 定义 MLP 层，明确指定 output_dim
        self.mlp1 = MLP_Block(input_dim=feature_dim,
                              output_dim=400,  # 指定输出维度
        ...
        # 融合模块，使用统一的 output_dim
        self.fusion_module = SumFusion(input_dim=400)  # 使用统一的维度
...
class SumFusion(nn.Module):
    def __init__(self, input_dim=400):
        ...
        self.fc_x = nn.Linear(input_dim, 400)  # 强制统一维度
        self.fc_y = nn.Linear(input_dim, 400)  # 强制统一维度
        self.output_layer = nn.Linear(400, 1)  # 最终输出
